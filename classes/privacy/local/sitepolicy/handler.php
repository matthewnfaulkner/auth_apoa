...
namespace auth_apoa\privacy\local\sitepolicy;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class handler extends \tool_policy\privacy\local\sitepolicy\handler {
    ...
    public static function signup_form($mform) {
        ...
        foreach (api::list_current_versions(policy_version::AUDIENCE_LOGGEDIN) as $policyversion) {
            if ($policyversion->optional == policy_version::AGREEMENT_COMPULSORY) {
                $url = new \moodle_url('/admin/tool/policy/view.php', array('policyid' => $policyversion->policyid));
                $mform->addElement('checkbox', 'policyagreed' . $policyversion->policyid, get_string('iagree', 'tool_policy',  '<a href="' . $url .
                    '" onclick="this.target=\'_blank\'">' .  $policyversion->name . '</a>'));
                $mform->addRule('policyagreed' . $policyversion->policyid, get_string('policyagree'), 'required', null, 'client');
                $compulsory[] = $policyversion->id;
            }
        }
    }
